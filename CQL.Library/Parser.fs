...
module Parser =
    ...
    //let expressions = choice [numberExpression;
    //                          stringExpression;
    //                          columnExpression;
    //                          boolExpression;
    //                          binaryEqualit]
    opp.TermParser <- (numberExpression <|> stringExpression <|> columnExpression <|> boolExpression .>> spaces) <|> between (spaces >>. pstring "(" >>. spaces) (spaces >>. pstring ")" >>. spaces) expr
    //do expressionRef := choice [numberExpression;
    //                                    stringExpression;
    //                                    columnExpression;
    //                                    boolExpression;
    //                                    binaryEquality]
...
//    let fromType, fromTypeRef = createParserForwardedToRef<From, unit>()
//    let joinType, joinTypeRef = createParserForwardedToRef<Join, unit>()
//    let comparisonPartType, comparisonPartTypeRef = createParserForwardedToRef<ComparisonExpression, unit>()
...
//    let columnExpression = betweenString "'" "'" specificColumn |>> ColumnExpression .>> spaces
//    let boolExpression : Parser<_,unit> = ((stringReturn "true" (Bool 1)) <|> (stringReturn "false" (Bool 0)))
//    let binaryEquality = betweenString "(" ")" (spaces >>. comparisonPartType .>> spaces .>>. (equal <|> notequal) .>>. (spaces >>. comparisonPartType)) |>> (fun ((x,y),z) -> Binary(y,x,z))

//    let manyEqualityExpression = 
//        let seperator = pstring "&&" .>> spaces 
//        sepBy1 binaryEquality seperator

//    let selectColumns = between (pstring "'") (pstring "'") (all <|> (specificColumn |>> Specifict))
//    let where = pstring "where" >>. spaces >>. binaryEquality .>> spaces
//    let manySelectParameter = 
//        let seperator = pstring "," .>> spaces 
//        sepBy1 selectColumns seperator
...
//    do comparisonPartTypeRef := choice [numberExpression;
//                                        stringExpression;
//                                        columnExpression;
//                                        boolExpression;
//                                        binaryEquality]
